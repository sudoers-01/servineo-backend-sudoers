const{Request,Response}=required('express');
const { delete_location_by_display_name,
        delete_many_locations_by_display_name,
        delete_location_by_place_id,
        delete_many_locations_by_place_id,
        delete_many_locations_by_query } = required('./delete_service');

async function attributeValidation(attributes){
    if(attributes == null){
            return res.status(400).json({ message: 'Missing parameter: list of attributes to change.'});
    }else{
        Object.keys(attributes).forEach(key => {
            if(key === "place_id"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: place_id.'});
                }
                if(!(/^[0-9]{7}$/.test(attributes[key]))){
                    return res.status(400).json({ message: 'Invalid parameter value: place_id must be numbers.'});
                }
            }
            else if(key === "licence"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: licence.'});
                }
            }else if(key === "osm_type"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: osm_type.'});
                }
            }else if(key === "osm_id"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: osm_id.'});
                }
                if(attributes[key].length != 10 || !(/^[0-9]{9}$/.test(attributes[key]))){
                    return res.status(400).json({ message: 'Invalid parameter value: osm_id.'});
                }
            }else if(key === "lat"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: lat.'});
                }
                if(!(/^-?\d+\.\d{1-10}/.test(attributes[key]))){
                    return res.status(400).json({ message: 'Invalid parameter value: lat.'});
                }
            }else if(key === "lon"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: lon.'});
                }
                if(!(/^-?\d+\.\d{1-10}/.test(attributes[key]))){
                    return res.status(400).json({ message: 'Invalid parameter value: lon.'});
                }
            }else if(key === "display_name"){
                if(attributes[key] === ""){
                    return res.status(400).json({ message: 'Missing parameter value: display_name.'});
                }
            }else if(key === "address"){
                if(attributes[key] == null){
                    return res.status(400).json({ message: 'Missing parameter value: address.'});
                }
            }else if(key === "boundingbox"){
                if(attributes[key].length === 0 && attributes[key].length !== 4){
                    return res.status(400).json({ message: 'Missing parameter value: boundingbox.'});
                }
                if(!(/^-?\d+\.\d{1-10}/.test(attributes[key][0])) && (!(/^-?\d+\.\d{1-10}/.test(attributes[key][1]))) &&
                    (!(/^-?\d+\.\d{1-10}/.test(attributes[key][2]))) && (!(/^-?\d+\.\d{1-10}/.test(attributes[key][3])))){
                    return res.status(400).json({ message: 'Invalid parameter value: boundingbox.'});
                }
            }
        });
    }
}


export async function deleteLocationByDisplayName(req, res){
    try{
        const { name } = req.query;
        if(!name){
            return res.status(400).json({error: 'Missing parameter: name'});
        }

    }catch(err){
        console.log('Error in controller while deleting location: ', err);
        res.status(500).json({error: 'Server error'});
    }
}

export async function deleteManyLocationsByDisplayName(req, res){
    try{
        const {name} = req.query;
          if(!name){
            return res.status(400).json({error: 'Missing parameter: name'});
        }
    }catch(err){
        res.status(500).json({error: 'Server error'});
    }
}

export async function deleteLocationByPlaceId(req, res){
    try{
        const { id } = req.query;
        if(!id){
            return res.status(400).json({error: 'Missing parameter: id'});
        }
        if(/^[0-9]{7}$/.test("id")){
            return res.status(400).json({error: 'Invalid parameter: id is not valid'});
        }
    }catch(err){
        res.status(500).json({error: 'Server error'});
    }
}
export async function deleteManyLocationsByPlaceId(req, res){
    try{
        const { ids } = req.query;
        if(!ids){
            return res.status(400).json({error: 'Missing parameter: ids'});

        }
        if(/^[0-9]{7}$/.test("ids")){
            return res.status(400).json({error: 'Invalid parameter: id is not valid'});
        }
    }catch(err){
        res.status(500).json({error: 'Server error'});
    }    
}

export async function deleteManyLocationsByQuery(req, res){
    try{
        const { query } = req.query;
        atributeValidation(query);
    }catch(err){
        res.status(500).json({error: 'Server error'});
    }
}
